...
{ 
	ifstream ReadPhase;
	ReadPhase.open("Data/input.dat");						//in input.dat scrivo fase da cui iniziare
  ReadPhase >> phase;
  folder = "Data/" + phase;
  folder = folder + "/";											//divido i risultati in 3 cartelle diverse in base alla fase
  
  char inst;
  ReadPhase >> inst;
  if(inst == 'y')	instant = true;
  else						instant = false;						//se devo stampare energia e pressione istantanea oppure no
  
  ReadPhase.close();
  
  Input(); //Inizialization
  
	ofstream OutEnergy,OutPressure;
	if(instant){
	  OutEnergy.open(folder + "instant_energy." + phase);
	  OutPressure.open(folder + "instant_pressure." + phase);
	}
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; ++istep)
    {
      ...
      if(instant){
	      double energy = walker[iv]/(double)npart + vtail;            //ATTENZIONE: bisogna aggiungere la correzione!!!
  	    OutEnergy << energy << endl;
  	    double pressure = rho * temp + (walker[iw] + (double)npart * ptail) / vol;             //Pressione (idem ^)
  	    OutPressure << pressure << endl;
      }
...
void Input()
{
  ifstream ReadPhase,ReadInput,ReadConf;
...
//Read input informations
  ...
  string file = folder + "input.";
  ReadInput.open(file + phase);
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Gofr.open(folder + "output.gofr.0",ios::app);	//media in ogni blocco di g(r)
    Gave.open(folder + "output.gave.0",ios::app); //media finale di g(r)
